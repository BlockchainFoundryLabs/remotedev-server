## Realtime monitoring
 ...
 By default, the websocket server is running on `ws://localhost:9999/socketcluster/`.  
 ### Messaging lifecycle
...
```
 ##### Python
```py
socket = Socketcluster.socket("ws://localhost:9999/socketcluster/") 
socket.connect()
```
 > Note that JavaScript client composes the url from `hostname` and `port`, adding `/socketcluster/` path automatically. For other clients, you should specify that path. For example, for `ObjectiveC` it would be `self.client.initWithHost("localhost/socketcluster/", onPort: 9999, securely: false)`.
